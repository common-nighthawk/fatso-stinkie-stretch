class InventoryShipmentAssignmentsController < ApplicationController
  ...
  def create
    @assignment = @inventory.create_assignment(inventory_shipment_assignment_params)
    redirect_to inventories_path
  end

  def update
    @assignment = @inventory.assignment.update_attributes(inventory_shipment_assignment_params)
    redirect_to inventories_path
  end
